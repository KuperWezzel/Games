from cards import fullDeck, Deck, Card, Suit
...
def main():
    ...
    cards_picked_up = 0
    moving = False
    moving_card: Optional[Card] = None
    moving_from = ""
    sg.theme('GreenTan')
    reset = False
    ...
    boardWE = [Deck() for _ in range(7)]
    emptyCard = Card(Suit("", "orange"), -1, "", (-1, -1), False)
    for i in range(7):
        card = stack.cards[0]
        for j in range(7-i):
            if j == 6-i:
                card.hidden = False
            card.location = (i, j)
            board[6 - i].moveToSelf([card])
        boardWE[6 - i] = board[6 - i].copy()
        for _ in range(12+7-i):
            boardWE[6 - i].addCard(emptyCard)
    Aces = {'Harten': Deck(), 'Klaveren': Deck(), 'Schoppen': Deck(), 'Ruiten': Deck()}
    x=boardWE[0].cards
    print(len(x), x)
    # card columns
    ...
    for i in range(7):
        col_j = []
        for j in range(12+1):
            col_j.append([sg.Text(boardWE[i].cards[j], key=f'cc~{i}~{j}', enable_events=True, s=(15, 1),
                                  text_color=boardWE[i].cards[j].colour(), metadata=boardWE[i].cards[j])])
    ...
    layout = [
        ...
        [
            sg.VSeparator(),
            cardColumn(card_col[0]), sg.VSeperator(),
            cardColumn(card_col[1]), sg.VSeperator(),
            cardColumn(card_col[2]), sg.VSeperator(),
            cardColumn(card_col[3]), sg.VSeperator(),
            cardColumn(card_col[4]), sg.VSeperator(),
            cardColumn(card_col[5]), sg.VSeperator(),
            cardColumn(card_col[6]), sg.VSeparator()
        ],
        [sg.HSeparator()],
        [
        ...
        if moving:
            for k in range(cards_picked_up):
                if k == 0:
                    ...
                    window[f"cc~{moving_from[1]}~{int(moving_from[-5:-3]) + k}"].update(background_color=sg.theme_background_color())
            moving = False
            moving_card = None
            moving_from = ""
            cards_picked_up = 0
        ...
        if not moving:
            ...
            for k in range(cards_picked_up):
                if k == 0:
                    ...
                    window[f"cc~{event[1]},0{int(event[-5:-3]) + k}"].update(background_color="lightgreen")
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            confirm = sg.Window("Reset?", layout=[[sg.Text("Weet je zeker dat je opnieuw wilt beginnen?")],
                                                  [sg.Button("Ja"), sg.Button("Nee")]])
            ...
            if moving:
                ...
                if event[1] == moving_card.suit.name[0] and len(Aces[event].cards) == moving_card.value - 1 and is_movable:
                    ...
                    if moving_from == "~open_stack~":
                        ...
                        window[moving_from].metadata = moving_card
            stop_moving()

        elif event[:3] == "cc~":
            print("cc", event[1], event[-5:-3])
            if moving and not card['h'] and card['f']:  # try placing card(s)
            ...
            elif moving and not card['h'] and not card['f'] and len(board[int(event[1])].cards) == 0:  # place at top of empty column
            ...
            elif not moving and not card['h'] and card['f']:  # pick up card(s)
            ...
            elif card['h'] and board[int(event[1])].cards[int(event[-5:-3])].is_bottom():  # hidden card
